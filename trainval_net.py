...
def model_loss(model, model_label):

    loss = torch.tensor(0.).cuda()
    for param, param_label in zip(model.parameters(), model_label.parameters()):
        loss += torch.norm(param-param_label.detach(), 2)
        # loss += F.l1_loss(param, param_label.detach())
...
if __name__ == '__main__':
  ...
  if args.net == 'vgg16':
    ...
    fasterRCNN_old = vgg16(imdb.classes, pretrained=True, class_agnostic=args.class_agnostic)
  ...
  fasterRCNN_old.create_architecture()
  ...
  if args.resume:
    ...
    fasterRCNN_old.load_state_dict(checkpoint['model'])
  ...
  if args.mGPUs:
    ...
    fasterRCNN_old = nn.DataParallel(fasterRCNN_old)
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    for step in range(iters_per_epoch):
      ...
      rois_label, cls_loss_cls, cls_entropy_loss, sim_loss_cls = fasterRCNN(im_data, im_info, gt_boxes, num_boxes)

      loss_model = model_loss(fasterRCNN, fasterRCNN_old)


      loss = 0.1*sim_loss_cls.mean() + 0.05*cls_loss_cls.mean() + 0.01*loss_model.mean()
      ...
      if step % args.disp_interval == 0:
        ...
        print("\t\t\tsim_loss_cls: %.4f, cls_loss_cls: %.4f, cls_entropy_loss: %.4f, loss_model %.4f" \
              % (sim_loss_cls.mean().item(), cls_loss_cls.mean().item(), cls_entropy_loss.mean().item(), loss_model.mean().item()))
        if args.use_tfboard:
          info = {
            'loss': loss_temp,
